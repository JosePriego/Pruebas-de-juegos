...
<style>
    ...
    .stMarkdown p { text-align: center; color: #444; font-family: 'Courier New', Courier, monospace; font-weight: bold; font-size: 13px;}
</style>
...
<script>
  ...
  function iniciarNivel(n) {
      ...
      if(n === 1) { jugador.vidas = modoDificil ? 3 : 5; puntuacion = 0; siguienteCaja = 100; jugador.armaTimer = 0; jugador.arma = 'normal'; arrancarMusica(); }
  ...
  function danarJefe(dmg) {
      ...
      if (jefe.hp <= 0) {
          ...
          if (nivel < 5) { estadoJuego = 'TRANSICION'; transicionTimer = 100; setTimeout(() => iniciarNivel(nivel + 1), 1500); } 
  ...
  function actualizarIAJefe() {
      ...
      if (nivel === 1) { 
          ...
          if (t === Math.floor(60 * velDisp)) { dispararJefe(jefe.x + 20, jefe.y + 30, -3, 0, 'bomba_fantasma'); dispararJefe(jefe.x + 20, jefe.y + 30, -1, 0, 'bomba_fantasma'); }
  ...
  function dibujar() {
    ...
    ctx.fillStyle = climaMalo ? "#2a3a4a" : "#4a5a6a"; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ...
    } else if (jugador.dashCooldown <= 0) { ctx.fillStyle = "#00ffff"; ctx.font = "bold 14px 'Courier New'"; ctx.fillText("💨 DASH LISTO", 10, uiY); }
    ...
    balas.forEach(b => { ctx.fillStyle = b.tipo==='laser' ? '#00ffff' : '#ffaa00'; ctx.fillRect(b.x, b.y, b.w, b.h); });
    ...
    if (estadoJuego === 'INICIO') {
        ...
        ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.font = "bold 24px 'Courier New'"; ctx.fillText("CLIC PARA ACTIVAR AUDIO Y EMPEZAR", canvas.width/2, canvas.height/2);
        ...
        ctx.fillStyle = "#ffd700"; ctx.textAlign = "center"; ctx.font = "bold 36px 'Courier New'"; ctx.fillText("¡MISIÓN CUMPLIDA!", canvas.width/2, canvas.height/2 - 40);
        ctx.fillStyle = "#fff"; ctx.font = "bold 20px 'Courier New'"; ctx.fillText("PUNTUACIÓN TOTAL: " + puntuacion, canvas.width/2, canvas.height/2 - 5);
        ctx.font = "bold 16px 'Courier New'"; ctx.fillText("[ESPACIO] Jugar Normal  |  [H] MODO DIFÍCIL", canvas.width/2, canvas.height/2 + 40);
    }
  }
  ...
  function bucle() {
    ...
    for (let i = secuaces.length - 1; i >= 0; i--) {
        ...
        if (s.x < dH.x + dH.w && s.x + s.w > dH.x && s.y < dH.y + dH.h && s.h + s.y > dH.y) {
            recibirDano(); explosiones.push({x: s.x, y: s.y, timer: 10, color: '#ff4500'}); secuaces.splice(i, 1); continue;
        }
    ...
    for (let i = cajas.length - 1; i >= 0; i--) {
        ...
        if (dH.x < c.x + 24 && dH.x + dH.w > c.x && dH.y < c.y + 24 && dH.h + dH.y > c.y) {
            if (c.tipo === 'vida') jugador.vidas++; else { jugador.armaTimer = 400; jugador.arma = c.tipo; } 
    ...
    for (let i = balas.length - 1; i >= 0; i--) {
        ...
        for (let j = secuaces.length - 1; j >= 0; j--) {
            ...
            if (b.x < s.x + s.w && b.x + b.w > s.x && b.y < s.y + s.h && b.h + b.y > s.y) {
                explosiones.push({x: s.x, y: s.y, timer: 5, color: '#ff4500'}); puntuacion += 5;
